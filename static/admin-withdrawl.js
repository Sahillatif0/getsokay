const host = "/admin";
let histories;
$.get(host + "/getwithdrawl",
    function (data, status) {
        histories = data.withdrawlhistory;
        histories.forEach(history=>{
            switch(history.status){
            case "PENDING":
                $("#btn"+history.withdrawID).css({"background": "#5681eb", "box-shadow": "#758c8f 2px 2px 2px 2px"});
                break;
            case "APPROVED":
                $("#btn"+history.withdrawID).css({"background": "#5beb56", "box-shadow": "#65a367 2px 2px 2px 2px"});
                break;
            case "REJECTED":
                $("#btn"+history.withdrawID).css({"background": "#d65c5c", "box-shadow": "#7b5259 2px 2px 2px 2px"});
                break;
            }
        })
    })
...
searchinput.addEventListener("input", (e) => {
    ...
    histories.forEach(history => {
        const historybox = document.getElementById(history.withdrawID);
        const isVisible = toString(history.withdrawID).includes(inputvalue) || history.username.toLowerCase().includes(inputvalue) || history.Account_Name.toLowerCase().includes(inputvalue) || history.Account_Number.toLowerCase().includes(inputvalue) || history.email.toLowerCase().includes(inputvalue) || history.status.toLowerCase().includes(inputvalue) || history.amount.toLowerCase().includes(inputvalue) || history.method.includes(inputvalue);
        if (isVisible === true) {
...
function filter() {
    ...
    temphistories.forEach(history=>document.getElementById(history.withdrawID).style.display = "none");
    ...
    temphistories.forEach(history=>document.getElementById(history.withdrawID).style.display = "flex");
}
